...
export async function upsertSession(sessionId: string, location: string, interests: string[]) {
  ...
  await supabase
    .from("sessions")
    .upsert({ id: sessionId, location, interests, updated_at: new Date().toISOString() });
}

export async function ensureSession(sessionId: string) {
  if (!hasDb || !supabase) {
    if (!mem.sessions.has(sessionId)) {
      mem.sessions.set(sessionId, { id: sessionId, location: null, interests: [] });
    }
    return;
  }
...
async function getSetIds(sessionId: string): Promise<string[]> {
  if (!hasDb || !supabase) {
    return [...mem.questSets.values()].filter((s) => s.sessionId === sessionId).map((s) => s.id);
  }
...
export async function getActiveQuestSet(sessionId: string): Promise<ActiveQuestSet | null> {
  if (!hasDb || !supabase) {
    const sets = [...mem.questSets.values()]
      .filter((s) => s.sessionId === sessionId)
      .sort((a, b) => b.createdAt - a.createdAt);
    if (sets.length === 0) return null;
    const set = sets[0];
    const quests = [...mem.quests.values()].filter((q) => q.quest_set_id === set.id);
  ...
  const { data: quests } = await supabase
    .from("quests")
    .select("id, title, description, cadence, difficulty, completed_at")
    .eq("quest_set_id", setRow.id);
  return {
...
export async function completeQuest(questId: string): Promise<Quest | null> {
  if (!hasDb || !supabase) {
    ...
    return { id: q.id, title: q.title, description: q.description, cadence: q.cadence, difficulty: q.difficulty, completed_at: q.completed_at };
  }
  const { data } = await supabase
    .from("quests")
    ...
    .select("id, title, description, cadence, difficulty, completed_at")
    .maybeSingle();
  if (data) return data as Quest;
  const { data: existing } = await supabase
    .from("quests")
    .select("id, title, description, cadence, difficulty, completed_at")
    .eq("id", questId)
    .maybeSingle();
  return (existing as Quest) ?? null;
}
...
export async function getCompletedQuests(sessionId: string): Promise<Quest[]> {
  if (!hasDb || !supabase) {
    return [...mem.quests.values()]
      ...
      .map((q) => ({ id: q.id, title: q.title, description: q.description, cadence: q.cadence, difficulty: q.difficulty, completed_at: q.completed_at }));
  }
  const setIds = await getSetIds(sessionId);
  if (setIds.length === 0) return [];
  const { data } = await supabase
...
export async function questBelongsToSession(questId: string, sessionId: string): Promise<boolean> {
  ...
  const { data } = await supabase
    .from("quests")
    .select("quest_set_id, quest_sets!inner(session_id)")
    .eq("id", questId)
    .maybeSingle();
